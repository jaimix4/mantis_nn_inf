...
model = load_model("/home/caballero/data_test/models_matlab/" + arguments[0])
...
emis = np.load('SOLPS-ITER/forward_emis_adas_goto_SOLPS-ITER_sims.npy')

# for this ratio nn from before
# 'emi_3-2','emi_5-2','728/706','728/668'
# '   0   ','   2   ','  11   ','  12   '

# this will output mean and std of the network

print(emis.shape)

# taking as input ratios with recombination
# input = emis[:, [0, 2, 11, 12]]

# taking as input ratios without recombination
input = emis[:, [0, 2, 6, 7]]

result_full = model(input)
...
# the shape I just know it tbh
result_mean = result_mean.reshape((98, 38, 7))
result_std = result_std.reshape((98, 38, 7))
print(result_mean.shape)
print(result_std.shape)

# file name to save
file_name = 'SOLPS-ITER/NOREC_nn_results_SOLPS-ITER_sims_' + arguments[0] + '.mat'
print(file_name)


io.savemat(file_name, \
    {'nn_outputs' : \
    {'Te_mean': result_mean[:, :, 0], \
        'ne_mean': result_mean[:, :, 1], \
            'no_mean': result_mean[:, :, 2], \
                'Irate_mean': result_mean[:, :, 3], \
                    'Rrate_mean': result_mean[:, :, 4], \
                        'rec_dom_mean': result_mean[:, :, 5],\
                            'Te_std': result_std[:, :, 0], \
                                'ne_std': result_std[:, :, 1], \
                                    'no_std': result_std[:, :, 2], \
                                        'Irate_std': result_std[:, :, 3], \
                                            'Rrate_std': result_std[:, :, 4], \
                                                'rec_dom_std': result_std[:, :, 5]} })


print("outputs saved on file: ", file_name)
